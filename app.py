...
if st.button("Generate"):
    ...
    if not prompt_simple.strip() and not prompt_thematic.strip():
        st.warning("Please enter at least one prompt.")
    else:
        with st.spinner("Generating outputs..."):

            # ---------------------------
            # Comparison 1: Simple sentences
            # ---------------------------
            if prompt_simple.strip():
            ...
            # ---------------------------
            # Comparison 2: Thematic QA
            # ---------------------------
            if prompt_thematic.strip():
